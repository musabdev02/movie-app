    ...
    <Ftu heading="Your Favorites">
      {
        favorites.length > 0 ? favorites.map((item: Movie) => <Moviecard key={item.id} movie={item} />) :
          <div className="w-full h-[60vh] flex items-center justify-center flex-col gap-3 text-gray-400">
            ...
            <p className="w-[25%] text-center mx-auto">You haven't saved any movies as favorites yet. To add a film to your personal collection, simply click the <Heart size={18} className="inline-block text-purple-400" /> icon.</p>
          </div>
      }
    </Ftu>
  )
